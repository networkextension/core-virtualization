...
$SSH root@"$IP" 'mkdir -p /usr/local/bin /usr/local/etc/rc.d'
$SCP "$AGENT" root@"$IP":/usr/local/bin/polar-agent
$SCP "$HERE/polar/polar_agent" "$HERE/polar/polar_seed" root@"$IP":/usr/local/etc/rc.d/
$SSH root@"$IP" '
set -e
chmod 0755 /usr/local/bin/polar-agent
chmod 0555 /usr/local/etc/rc.d/polar_agent /usr/local/etc/rc.d/polar_seed
sysrc -q polar_seed_enable=YES polar_agent_enable=YES
mkdir -p /root/.polar
